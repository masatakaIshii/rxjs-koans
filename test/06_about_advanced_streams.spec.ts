...
test('merging', () => {
    ...
    expect(easy.join(' ')).toBe('1 2 3 A B C')
})
...
test('merging events', () => {
    ...
    expect(both.join(' ')).toBe('I am nobody. Nobody is perfect.')
...
test('concat events work only for the first event if it is still not completed', () => {
    ...
    expect(both.join(' ')).toBe('I am perfect.')
    expect(first.join(' ')).toBe('I am perfect.')
})
...
test('concat events work with two events if first event is completed', () => {
    ...
    expect(both.join(' ')).toBe('I am nobody. Nobody is')
    expect(first.join(' ')).toBe('I am')
})
...
test('need to subscribe immediately when splitting', function () {
    ...
    split.subscribe((group) => {
        group.pipe(
            toArray(),
            map((value) => value.reduce((acc, number) => acc + number) / value.length)        
